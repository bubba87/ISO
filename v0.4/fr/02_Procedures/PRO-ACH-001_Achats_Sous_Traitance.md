# Procédure Achats et Sous-Traitance

| | |
|---|---|
| **Référence** | PRO-ACH-001 |
| **Version** | 0.4 |
| **Date de création** | 10/02/2026 |
| **Date de revision** | 18/02/2026 |
| **Rédigé par** | Roxane Wicky |
| **Approuve par** | Roxane Wicky |

> **Légende :** :red_circle: [À REMPLIR] = obligatoire, manquant | :yellow_circle: [RECOMMANDE] = recommandé | :green_circle: = déjà rempli | :blue_circle: [À VÉRIFIER] = a confirmer

---

## 1. Objet

Définir les règles de sélection, d'évaluation et de maîtrise des fournisseurs et sous-traitants, en particulier les partenaires chinois qui fabriquent les moules et les pièces plastiques injectees pour les clients européens de Plus Sarl.

> **Cette procédure est CRITIQUE pour Plus Sarl.** La fabrication étant entièrement
> externalisée auprès de partenaires en Chine, la maîtrise de ce processus est essentielle
> et sera examinée en détail par l'auditeur de certification (SQS).

## 2. Domaine d'application

Tous les achats et prestations externalisées ayant un impact sur la qualité des produits livrés aux clients (~10 clients actifs) :
- Fabrication de moules (partenaires chinois)
- Injection de pièces plastiques (partenaires chinois)
- Fabrication de vis (Whang, Yuyao)
- Sous-traitance de specialites par les partenaires chinois
- Transport international (transitaire, transporteur aérien/maritime/ferroviaire)
- Tout autre fournisseur ayant un impact qualité

## 3. Responsabilités

| Responsabilité | Qui |
|---|---|
| Sélection et maintien des partenaires | :green_circle: Roxane Wicky, gérante |
| Évaluation continue de la performance | :green_circle: Roxane Wicky, gérante |
| Emission et suivi des commandes | :green_circle: Roxane Wicky, gérante |
| Coordination technique avec les partenaires | :green_circle: Roxane Wicky, gérante |
| Suivi de production a distance | :green_circle: Roxane Wicky, gérante |

## 4. Partenaires de fabrication

### 4.1 Partenaire principal : Yuyao Mould Factory

| Élément | Détail |
|---|---|
| **Nom** | :green_circle: Yuyao Mould Factory |
| **Localisation** | :green_circle: Chine |
| **Partenariat depuis** | :green_circle: 2007 (depuis la création de Plus Sarl) |
| **Classe** | :green_circle: A - Critique |
| **Produits/Services** | :green_circle: Fabrication de moules, injection de pièces plastiques |
| **Rôle élargi** | :green_circle: Sélectionné et qualifié des sous-traitants quand nécessaire |
| | Évalué la faisabilité technique des projets |
| | Assure le suivi de la qualité en production |
| **Base de la relation** | :green_circle: Confiance construite sur pres de 20 ans de collaboration |
| **NC documentées** | :green_circle: NC_2026_1001 - Erreur d'étiquetage (90.60.05710L au lieu de 90.60.05710) - 10/02/2026 |

### 4.2 Second partenaire spécialisé : Whang

| Élément | Détail |
|---|---|
| **Nom** | :green_circle: Whang |
| **Localisation** | :green_circle: Yuyao, Chine |
| **Classe** | :green_circle: A - Critique |
| **Produits/Services** | :green_circle: Vis (screws) |
| **Sous-traitance** | :green_circle: Ne sous-traite pas -- production interne uniquement |
| **Certifications** | :blue_circle: [À VÉRIFIER -- incertain] |

### 4.3 Transporteurs et transitaires

| Élément | Détail |
|---|---|
| **Classe** | :green_circle: B - Important |
| **Modes de transport utilisés** | :green_circle: Aérien, maritime, ferroviaire |
| **Critères** | :green_circle: Fiabilité, respect des délais, couverture géographique |
| **Noms des transporteurs** | :yellow_circle: [RECOMMANDE -- noms non disponibles. Il est recommandé de documenter les noms des transporteurs et transitaires utilisés pour assurer la traçabilité et l'évaluation] |

## 5. Classification des fournisseurs

| Classe | Critère | Exemples | Niveau de maîtrise |
|---|---|---|---|
| **A - Critique** | Impact direct sur la qualité du produit | :green_circle: Yuyao Mould Factory, Whang | Suivi continu, évaluation régulière |
| **B - Important** | Impact indirect sur la qualité ou les délais | Transporteurs, transitaires :yellow_circle: [RECOMMANDE -- noms a documenter] | Évaluation périodique simplifiée |
| **C - Standard** | Impact faible sur la qualité | :green_circle: Fournitures de bureau, fiduciaire Paradiso, outils informatiques | Pas d'évaluation formelle |

## 6. Sélection et maintien des partenaires

### 6.1 Critères de sélection et de maintien

La sélection et le maintien des partenaires de Plus Sarl reposent sur les critères suivants :

> **Source :** UPDATE 7.5-8.7 — Maîtrise des partenaires et fournisseurs industriels (21/02/2026) 🟢

| Critère | Importance | Commentaire |
|---|---|---|
| **Expérience industrielle et compétence technique** | Critique | Savoir-faire démontré dans le domaine concerné |
| **Capacité à produire selon les spécifications clients** | Critique | Conformité aux exigences techniques |
| **Qualité des installations et conditions de travail observées** | Élevée | Évaluée lors de visites sur site |
| **Fiabilité et stabilité des délais** | Élevée | Livraisons dans les temps convenus |
| **Capacité à respecter les délais** | Élevée | Délais de production tenus |
| **Réactivité et flexibilité** | Moyenne | Échanges fluides par email et WeChat |
| **Cohérence des conditions tarifaires** | Moyenne | Compétitivité et transparence |
| **Confiance construite dans le temps** | Critique | Relation de long terme, fiabilité démontrée |

Le partenaire industriel principal, en relation historique et durable avec l'entreprise, participe à l'identification et à la sélection de nouveaux sous-traitants lorsque nécessaire.

Des visites sur site peuvent être réalisées afin d'évaluer les capacités techniques et organisationnelles.

> **Note :** Les partenaires actuels sont en place depuis la création de Plus Sarl en 2007 (Yuyao Mould Factory) et sont maintenus sur la base de la confiance mutuelle, de la qualité des produits et du respect des délais. Le suivi de performance est continu.

### 6.2 Sélection d'un nouveau partenaire (si nécessaire)

| # | Étape | Description |
|---|---|---|
| 1 | Identification | Recherche de partenaires potentiels (réseau, recommandations, salons) |
| 2 | Évaluation initiale | Évaluation de la capacité technique, des références, des échantillons |
| 3 | Essai | Commande test sur un premier projet |
| 4 | Validation | Validation sur la base des résultats (qualité, délai, communication) |
| 5 | Intégration | Intégration dans le processus de commande |

## 7. Accord qualité avec les partenaires chinois

> **[POINT D'ATTENTION]** : A ce jour, la relation avec les partenaires chinois repose sur la confiance
> mutuelle construite depuis 2007. Il n'existe pas d'accord qualité formel signe.
>
> **Échéance :** :green_circle: Des que le dossier de certification est pret, un accord qualité sera formalisé avec Yuyao Mould Factory et Whang.
>
> **Recommandation :** La formalisation d'un accord qualité avec les partenaires de classe A est
> fortement recommandée pour répondre aux exigences de la norme ISO 9001:2015 (clause 8.4).
> Cet accord devrait couvrir au minimum les points suivants :

### Contenu minimum recommandé de l'accord qualité

| Clause | Description | Statut |
|---|---|---|
| 1. Objet | Description des produits/services concernés | [ ] A formaliser |
| 2. Exigences qualité | Specifications générales (tolérances, materiaux, finitions) | [ ] A formaliser |
| 3. Documentation technique | Le partenaire doit recevoir les plans/CDC avant fabrication | [ ] Pratique en place |
| 4. Contrôle en cours de fabrication | Le partenaire effectué des contrôles et fournit des rapports | [ ] A formaliser |
| 5. Contrôle final et echantillonnage | Le partenaire effectué un contrôle final avant expédition et envoie un echantillonnage chez Plus Sarl en parallèle du transport | [ ] A formaliser |
| 6. Rapport d'inspection | Le partenaire fournit un rapport d'inspection avec chaque livraison | [ ] A formaliser |
| 7. Droit d'audit / visite | Plus Sarl peut visiter l'usine | [ ] A formaliser (visite planifiée mars 2026) |
| 8. Gestion des NC | Processus de traitement des NC (délais, responsabilités) | [ ] A formaliser |
| 9. Traçabilité | Le partenaire assure la traçabilité des lots | [ ] A formaliser |
| 10. Modifications | Toute modification doit être soumise et approuvee par Plus Sarl | [ ] Pratique en place |
| 11. Confidentialité | Protection des plans, spécifications et données clients | [ ] A formaliser |

> **Action planifiée :** Un voyage en Chine est planifié en mars 2026 pour formaliser le suivi
> de production, renforcer les exigences qualité et traiter les actions correctives en cours
> (notamment AC_2026_001 liée à l'erreur d'étiquetage NC_2026_1001).

## 8. Contrôle qualité avant expédition

:green_circle: Le partenaire realisera un contrôle qualité avant expédition et enverra un echantillonnage chez Plus Sarl en parallèle du transport chez le client européen.

| Élément | Détail |
|---|---|
| **Responsable du CQ** | :green_circle: Partenaire chinois (Yuyao Mould Factory ou Whang) |
| **Moment du contrôle** | :green_circle: Avant expédition |
| **Echantillonnage** | :green_circle: Le partenaire envoie des échantillons chez Plus Sarl en parallèle de l'expédition au client européen |
| **Objectif** | Permettre a Plus Sarl de vérifier la conformité des produits en parallèle de la livraison |
| **Documentation** | :red_circle: [À REMPLIR -- le format du rapport de CQ et la taille de l'echantillonnage doivent être définis dans l'accord qualité] |

> **Note :** Ce processus de CQ pre-expédition avec echantillonnage parallèle constitue un élément cle de la maîtrise de la qualité pour Plus Sarl, qui ne réceptionne pas directement les produits. Il sera formalisé dans l'accord qualité avec chaque partenaire.

## 9. Fiche de production FileMaker

> **Source :** UPDATE 2.1 — Processus 02 Achats & Sous-traitance (18/02/2026) 🟢

Pour chaque commande, une **fiche de production** est créée dans le système FileMaker comprenant :

| Élément | Description | Statut |
|---|---|---|
| **Caracteristiques attendues du produit** | Specifications techniques, materiaux, tolérances, dimensions | 🟢 Enregistré dans FileMaker |
| **Délai de fabrication** | Délai convenu avec le partenaire chinois | 🟢 Enregistré dans FileMaker |
| **Mises a jour de la fiche** | Modifications en cours de production (ajustements techniques, délais) | 🟢 Mis à jour dans FileMaker |
| **Suivi de production** | Échanges réguliers avec les partenaires industriels (e-mail, WeChat) | 🟢 Traçabilité email/WeChat |
| **Echantillonnage** | Envoi d'échantillons aux clients et a Plus Sarl lorsque nécessaire pour validation | 🟢 |

> Cette fiche assure la traçabilité complète de la production et constitue une preuve de suivi pour l'audit SQS.

---

## 10. Processus de commande

### 10.1 Logigramme

```
    Demande client reçue
            |
            v
    +---------------------------+
    | 1. Revue de commande      |---> Analyse selon M1-DIR-001 :
    |    (cf. CTX-QUA-001       |     besoins (O1), faisabilité (O2),
    |     section 1.3)          |     qualité (O4)
    +---------------------------+
            |
            v
    +---------------------------+
    | 2. Enregistrer dans       |---> Création fiche commande +
    |    FileMaker              |     fiche de production
    +---------------------------+
            |
            v
    +---------------------------+
    | 3. Coordination technique |---> Échanges par email et WeChat
    |    avec le partenaire     |     avec Yuyao Mould Factory ou
    |    chinois                |     Whang
    +---------------------------+
            |
            v
    +---------------------------+
    | 4. Échanges iteratifs     |---> Ajustements techniques,
    |                           |     validation de la faisabilité,
    |                           |     confirmation des délais et prix
    +---------------------------+
            |
            v
    +---------------------------+
    | 5. Etablir les documents  |---> Facture douaniere
    |    de commande            |     Facture client
    |                           |     Bon de livraison
    |                           |     Accusé de réception
    +---------------------------+
            |
            v
    +---------------------------+
    | 6. Lancement de la        |---> Production chez le partenaire
    |    production             |     chinois
    +---------------------------+
            |
            v
    +---------------------------+
    | 7. Suivi de production    |---> Points d'étape (photos, rapports)
    |    a distance             |     via email/WeChat
    +---------------------------+
            |
            v
    +---------------------------+
    | 7b. Controle qualité      |---> CQ par le partenaire avant
    |     pre-expédition        |     expédition + envoi échantillons
    |                           |     chez Plus Sarl en parallèle
    +---------------------------+
            |
            v
    +---------------------------+
    | 8. Documents d'export     |---> Documents douaniers,
    |                           |     certificats d'origine, etc.
    +---------------------------+
            |
            v
    +---------------------------+
    | 9. Transport              |---> Aerien / Maritime / Ferroviaire
    |                           |     selon urgence et volume
    +---------------------------+
            |
            v
    +---------------------------+
    | 10. Suivi de livraison    |---> Tracking en ligne, contact
    |                           |     transporteur
    +---------------------------+
            |
            v
    +---------------------------+
    | 11. Confirmation client   |---> Le client confirme la réception
    +---------------------------+
            |
            v
    +---------------------------+
    | 12. Facturation           |---> Emission de la facture finale
    +---------------------------+
            |
       Reclamation client ?
       /                \
     NON                OUI
      |                   |
      v                   v
  Commande           PRO-NCF-001
  soldée             (non-conformité)
```

### 10.2 Rôle spécifique de Yuyao Mould Factory

En tant que partenaire principal depuis 2007, Yuyao Mould Factory joue un rôle élargi :

| Fonction | Description |
|---|---|
| **Évaluation de faisabilité** | Analyse des plans et spécifications clients pour confirmer la faisabilité technique |
| **Sélection de sous-traitants** | Lorsque le projet nécessité des compétences spécifiques, Yuyao Mould Factory sélectionné et qualifié les sous-traitants adaptes |
| **Suivi qualité en production** | Assure le monitoring de la qualité pendant la fabrication, y compris chez ses sous-traitants |
| **Coordination technique** | Interface technique entre Plus Sarl et les ateliers de production |
| **CQ pre-expédition** | :green_circle: Réalisé un contrôle qualité avant expédition et envoie un echantillonnage chez Plus Sarl en parallèle du transport |

### 10.3 Rôle de Whang

| Fonction | Description |
|---|---|
| **Production spécialisée** | :green_circle: Fabrication de vis (screws) |
| **Production interne uniquement** | :green_circle: Ce partenaire ne sous-traite pas -- toute la production est réalisée en interne |
| **Certifications** | :blue_circle: [À VÉRIFIER -- incertain] |
| **CQ pre-expédition** | :green_circle: Réalisé un contrôle qualité avant expédition et envoie un echantillonnage chez Plus Sarl en parallèle du transport |

## 11. Évaluation continue des partenaires

### 11.1 Suivi continu (partenaires de classe A)

La performance des partenaires chinois est évaluée de manière continue sur la base :

> **Source :** UPDATE 7.5-8.7 — Évaluation et suivi des fournisseurs (21/02/2026) 🟢

| Critère | Mode de suivi |
|---|---|
| **Respect des délais de production** | Comparaison délai annoncé vs. délai réel de livraison |
| **Niveau de qualité des produits livrés** | Suivi des réclamations clients, taux de NC par partenaire |
| **Taux de non-conformités et réactivité en cas de problème** | Délai et efficacité de traitement des NC signalées |
| **Stabilité des conditions commerciales** | Suivi des prix et conditions tarifaires |

En cas de dysfonctionnements répétés, une analyse est réalisée et une décision peut être prise de chercher un partenaire alternatif. Compte tenu de la nature spécifique des productions (petites séries techniques), le changement de fournisseur fait l'objet d'une évaluation préalable de faisabilité et de disponibilité.

> 🟡 [RECOMMANDÉ] Un renforcement formel du suivi fournisseur est en cours de structuration afin d'améliorer la traçabilité des évaluations.

> **Premier cas documenté de NC :** La NC_2026_1001 (erreur d'étiquetage sur SHIP_25058/CFM00057428/90.60.05710) constitue le premier cas documenté de non-conformité avec le fournisseur actuel. Le fournisseur a été informe via WeChat le 10/02/2026. Le traitement est en cours et sera renforce lors du voyage en Chine prévu en mars 2026.

### 11.2 Évaluation périodique formelle

| Classe | Frequence d'évaluation |
|---|---|
| A - Critique (partenaires chinois) | Annuelle (formelle) + suivi continu |
| B - Important (transporteurs) | Annuelle (simplifiée) |
| C - Standard | Pas d'évaluation formelle |

Voir formulaire FOR-EVF-001 pour le détail de l'évaluation annuelle.

### 11.3 Suivi de la qualité de production sur site

> **Voyage en Chine planifié en mars 2026 :**
> Un voyage chez les partenaires chinois est planifié en mars 2026 avec les objectifs suivants :
> - Renforcer le suivi de la qualité de production sur site
> - Traiter les actions correctives en cours (AC_2026_001 - erreur d'étiquetage)
> - Discuter la mise en place d'une checklist de contrôle qualité
> - Avancer sur la formalisation de l'accord qualité (cf. section 7)
> - Vérifier les conditions de production, d'étiquetage et d'emballage
> - Renforcer les exigences sur la vérification des etiquettes avant expédition
> - Formaliser le processus de CQ pre-expédition et d'echantillonnage (cf. section 8)

### 11.4 Actions selon le résultat

| Note globale | Appreciation | Action |
|---|---|---|
| >= 80% | Partenaire performant | Maintien de la relation |
| 60-79% | Partenaire acceptable | Discussion des points d'amélioration |
| < 60% | Partenaire insuffisant | Plan d'amélioration ou recherche d'alternative |

## 12. Liste des fournisseurs approuves

| Fournisseur | Classe | Produit/Service | Pays | Accord qualité | Dernière évaluation | Note | Statut |
|---|---|---|---|---|---|---|---|
| :green_circle: Yuyao Mould Factory | A | Moules + injection + coordination sous-traitance | Chine | Non (base confiance) [A FORMALISER -- des que le dossier de certification est pret] | :red_circle: [À REMPLIR] | :red_circle: [À REMPLIR] | Approuve |
| :green_circle: Whang | A | Vis (screws) -- production interne | Yuyao, Chine | Non (base confiance) [A FORMALISER -- des que le dossier de certification est pret] | :red_circle: [À REMPLIR] | :red_circle: [À REMPLIR] | Approuve |
| :yellow_circle: [RECOMMANDE -- noms a documenter] | B | Transport international (aérien/maritime/ferroviaire) | :yellow_circle: [RECOMMANDE] | N/A | :red_circle: [À REMPLIR] | :red_circle: [À REMPLIR] | Approuve |
| :green_circle: Paradiso | C | Fiduciaire / comptabilite | Suisse | N/A | N/A | N/A | Approuve |

---

## 12bis. Validation des nouveaux outillages (clause 8.4)

> **Source :** UPDATE 7.5-8.7 — Validation outillages (21/02/2026) 🟢

Lors du développement d'un nouveau moule ou d'une modification d'outillage :
- les spécifications techniques sont validées avec le client
- un échantillon ou une première production peut être soumis à validation
- la production série ne débute qu'après confirmation du client

Les validations sont conservées sous forme d'échanges écrits par courriel.

## 12ter. Confidentialité et propriété intellectuelle (clause 8.4)

> **Source :** UPDATE 7.5-8.7 — Confidentialité PI (21/02/2026) 🟢

Les designs, plans et spécifications techniques appartiennent aux clients et sont traités comme confidentiels.

Les moules développés pour les clients sont stockés chez les partenaires industriels en Chine.

Les informations techniques sont transmises uniquement aux partenaires concernés et ne sont pas diffusées sans autorisation.

---

> **Instructions de remplissage :**
> 1. **PRIORITÉ** : Formaliser un accord qualité avec Yuyao Mould Factory et Whang des que le dossier de certification est pret
> 2. Conservez TOUS les emails et échanges WeChat avec les partenaires chinois -- ce sont des preuves de suivi
> 3. Enregistrez toutes les commandes dans FileMaker avec les références, délais et spécifications
> 4. L'auditeur (SQS) voudra voir : les preuves de coordination technique, les évaluations, le suivi des NC par partenaire
> 5. Le voyage en Chine de mars 2026 est une opportunité pour formaliser les accords qualité, renforcer le suivi de production et traiter AC_2026_001
> 6. Les designs et plans appartiennent aux clients -- Plus Sarl ne fait pas de conception (exclusion 8.3)
> 7. Les moules sont la propriété des clients ; un inventaire existe (cf. PRO-DOC-001, section 7)
> 8. Le CQ pre-expédition avec echantillonnage parallèle est un élément cle a formaliser dans l'accord qualité

---

## Historique des revisions

| Version | Date | Description de la modification | Auteur |
|---|---|---|---|
| 0.1 | 10/02/2026 | Création initiale | Roxane Wicky |
| 0.2 | 10/02/2026 | Ajout de la référence a NC_2026_1001 comme premier cas documenté de NC fournisseur. Ajout de la section 9.3 (suivi qualité sur site) avec le voyage en Chine planifié en mars 2026 et ses objectifs. Mise à jour de la section 7 (accord qualité) avec référence au voyage de mars 2026. | Roxane Wicky |
| 0.3 | 12/02/2026 | Intégration des réponses (Whang, Paradiso, SQS, cloud, echantillonnage CQ). Ajout du système de légende des champs. | Roxane Wicky |
| 0.4 | 18/02/2026 | Intégration UPDATE 2.1 : ajout section 9 "Fiche de production FileMaker" (caracteristiques, délais, mises a jour, suivi, echantillonnage). Ajout de la revue de commande (cf. M1-DIR-001, CTX-QUA-001 section 1.3) dans le logigramme. Renumerotation des sections 9→10, 10→11, 11→12. | Roxane Wicky |
| 0.4 | 21/02/2026 | Intégration UPDATE 7.5-8.7 (clause 8.4) : mise à jour des 7 critères de sélection partenaires, 4 indicateurs d'évaluation continue, renforcement formel du suivi fournisseur, validation des nouveaux outillages (échantillon avant série), confidentialité et propriété intellectuelle. | Roxane Wicky |
