#!/usr/bin/env python3
"""
DOCX Generator for the ISO 9001:2015 audit dossier — Plus Sarl
Combines MQ-001 + INDEX into a single .docx file for submission to SQS.
Uses python-docx for professional rendering.
"""
...
def add_cover_page(doc):
    ...
    add_formatted_run(p, "AUDIT DOSSIER", bold=True, size=28, color=BLUE_DARK)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_formatted_run(p, "Part 1 — Quality Manual (MQ-001)", size=10, color=GRAY)

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_formatted_run(p, "Part 2 — Audit Dossier Index", size=10, color=GRAY)

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_formatted_run(p, "Document prepared for the certification body SQS", size=9, color=GRAY, italic=True)
...
def main():
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_formatted_run(p, "PART 1", bold=True, size=14, color=BLUE_DARK)
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_formatted_run(p, "Quality Manual — MQ-001", bold=True, size=18, color=BLUE_DARK)
    doc.add_paragraph()
    ...
    # === Part 2: Audit Dossier Index ===
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_formatted_run(p, "PART 2", bold=True, size=14, color=BLUE_DARK)
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_formatted_run(p, "Audit Dossier Index", bold=True, size=18, color=BLUE_DARK)
    doc.add_paragraph()
    ...
    # === Save ===
    output_path = os.path.join(SCRIPT_DIR, 'AUDIT_DOSSIER_ISO9001_Plus_Sarl_v0.4.docx')
